...
void pq_insert(struct tnode* p)
{
    ...
    // Traverse to find the right position (sorted by freq)
    ...
    // Case 2: Insert before the head (i.e., smallest freq)
    if(prev == NULL)  // i.e., p < qhead
    {
        p->next = qhead;
        qhead = p;
    }
    ...
    {
        // Case 3: Insert between prev and curr (middle or end)
        prev->next = p;
        p->next = curr;
    }
}
...
struct tnode* pq_pop()
{
    ...
    qhead = qhead->next; // move head to next node
    p->next = NULL;      // disconnect popped node
    printf("popped: %c, %f\n", p->symbol, p->freq);
    return p;
}
...
void generate_code(struct tnode* root, int depth)
{
    ...
    if (root->isleaf)
    {
        symbol = root->symbol;
        len = depth;
        code[symbol][len] = '\0';  // Null-terminate the string
